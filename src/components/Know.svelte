<script>
    ...
    import allFramework from "./../../public/data/farmework.json"

    let indexType = 0

    let types = ["Frontend", "Backend", "Mobile", "language"]
    ...
    <div class="w-full h-full my-6 tabs-sm flex items-center justify-start flex-col lg:p-10" id="Stack">
        <div class="taps lg:m-5">
            {#each types as type , index}
                <button on:click={()=>{indexType = -5 ; indexType = index  }} class={`tab tab-sm lg:tag-lg tab-bordered text-lg font-medium ${indexType === index && "tab-active"}`}>{type}</button>
            {/each}
        </div>
